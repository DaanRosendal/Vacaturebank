...
<?php
    if(isset($_POST["submit"])){
        if(isset($_GET["token"]) && isset($_GET["timestamp"])){
            ...
            try {
               $sql = "SELECT * FROM gebruikers WHERE email = ? AND token = ?";
               ...
               if ($r["email"] == $email){
                   ...
                   if($dif <= 43200) {
                       ...
                       if($stmt){
                          echo "<script>alert('Uw wachtwoord is veranderd.'); location.href='inloggen-w.php'</script>";
                       }
                   } else {
                    echo "<script>alert('Uw link is verlopen. Vraag opnieuw een link aan.'); location.href='wachtwoord-vergeten-w.php'</script>";
                   }
               } else {
