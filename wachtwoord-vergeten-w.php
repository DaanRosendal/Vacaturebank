...
<body>
    <div class="container col-centered">
        <div class="page-header text-center unselectable">
            <h1 class="mt-2 display-4">Wachtwoord vergeten</h1>
    ...
    if (isset($_POST["submit"])){
        ...
        if ($r["email"] == $email){
            ...
            try {
                $sql = "UPDATE gebruikers SET token = ? WHERE email = ?";
                $stmt = $verbinding->prepare($sql);
            ...
            $url = sprintf("%s://%s",isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] != 'off'?'https' : 'http',$_SERVER['HTTP_HOST'].dirname($_SERVER["PHP_SELF"])."/wachtwoord-resetten-w.php");
